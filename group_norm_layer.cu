...
void GroupNormLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // compute mean
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_ * group_ratio_, group_num_ * spatial_dim,
        1. / (group_num_ * spatial_dim), bottom_data, group_sum_multiplier_.gpu_data(), 0.,
        mean_.mutable_gpu_data());

  // subtract mean
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,num_ * group_ratio_,
      group_num_ * spatial_dim, 1, -1., mean_.gpu_data(),
      group_sum_multiplier_.gpu_data(), 1., top_data);
 
  // compute variance using var(X) = E((X-EX)^2)
  caffe_gpu_mul(top[0]->count(), top[0]->gpu_data(), top[0]->gpu_data(),temp_.mutable_gpu_data());  // (X-EX)^2
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_ * group_ratio_, group_num_ * spatial_dim,
        1. / (group_num_ * spatial_dim), temp_.gpu_data(),
        group_sum_multiplier_.gpu_data(), 0.,variance_.mutable_gpu_data());
  ...
  // div variance    
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_ * group_ratio_,
      group_num_ * spatial_dim, 1, 1., variance_.gpu_data(),
      group_sum_multiplier_.gpu_data(), 0., temp_.mutable_gpu_data());
...
void GroupNormLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_ * group_ratio_, group_num_ * spatial_dim, 1.,
      bottom_diff, group_sum_multiplier_.gpu_data(), 0.,
      mean_.mutable_gpu_data());
  
  // reshape (broadcast) the above
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_ * group_ratio_,
      group_num_ * spatial_dim, 1, 1., mean_.gpu_data(),
      group_sum_multiplier_.gpu_data(), 0., bottom_diff);
  ...
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_ * group_ratio_, group_num_ * spatial_dim, 1.,
      top_diff, group_sum_multiplier_.gpu_data(), 0.,
      mean_.mutable_gpu_data());
  ...
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_ * group_ratio_,
      group_num_ * spatial_dim, 1, 1., mean_.gpu_data(),
      group_sum_multiplier_.gpu_data(), 1., bottom_diff);
